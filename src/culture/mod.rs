...
use crate::lunar::{LunarDay, LunarMonth};
...
impl God {
    ...
    pub fn get_day_gods_slice(month: SixtyCycle, day: SixtyCycle) -> &'static [u8] {
        let month_index = month.get_earth_branch().next(-2).get_index();
        let day_index = day.get_index();

        DAY_GODS_TABLE
            .get(month_index)
            .and_then(|month_data| month_data.get(day_index))
    ...
    pub fn get_day_gods(month: SixtyCycle, day: SixtyCycle) -> Vec<Self> {
        Self::get_day_gods_slice(month, day)
            .iter()
            .map(|&byte| Self::from_index(byte as isize))
            .collect()
    }
}
...
impl Phase {
    ...
    fn get_start_solar_time(&self) -> SolarTime {
        ...
        loop {
            let t: f64 = Sxtwl::m_sa_lon_t((n + i) as f64 * PI_2) * 36525.0;
            if !JulianDay::from_julian_day(jd + t - Sxtwl::dtt(t))
                .get_solar_day()
                .is_before(d)
            {
                break;
        ...
        let t: f64 = Sxtwl::m_sa_lon_t(
            (n as f64 + i as f64 + [0, 90, 180, 270][self.get_index() / 2] as f64 / 360.0) * PI_2,
        ) * 36525.0;
        JulianDay::from_julian_day(jd + t - Sxtwl::dtt(t)).get_solar_time()
    }
