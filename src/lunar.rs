...
lazy_static! {
    static ref LEAP_MONTH_YEAR_DATA: Vec<Vec<isize>> = {
...
impl LunarYear {
    ...
    pub fn get_leap_month(&self) -> usize {
        ...
        for i in 0..LEAP_MONTH_YEAR_DATA.len() {
            if LEAP_MONTH_YEAR_DATA[i].contains(&self.year) {
                return i + 1;
            }
        }
        0
...
impl LunarMonth {
    pub fn new(year: isize, month: isize) -> Result<Self, String> {
        ...
        // 冬至前的初一，今年首朔的日月黄经差
        let mut w: f64 = Sxtwl::calc_shuo(dong_zhi_jd);
        if w > dong_zhi_jd {
            w -= 29.53;
        }
        ...
        let first_day: f64 = Sxtwl::calc_shuo(w);
        let first_julian_day: JulianDay = JulianDay::from_julian_day(J2000 + first_day);
        // 本月天数 = 下月初一 - 本月初一
        let day_count: usize = (Sxtwl::calc_shuo(w + 29.5306) - first_day) as usize;
    ...
    pub fn get_nine_star(&self) -> NineStar {
        ...
        NineStar::from_index(
            27 - self.year.get_sixty_cycle().get_earth_branch().get_index() as isize % 3 * 3
                - index,
        )
    }
        ...
        if n == 0 {
            ...
            if n > 0 {
                ...
                while d >= week_count {
                    d -= week_count;
                    m = m.next(1);
                    if LunarDay::from_ymd(m.get_year(), m.get_month_with_leap(), 1).get_week()
                        != self.start
                    {
                        d += 1;
                    }
