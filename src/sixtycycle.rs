    ...
    pub fn get_ten(&self) -> Ten {
        Ten::from_index(
            (self.get_heaven_stem().get_index() as isize
                - self.get_earth_branch().get_index() as isize)
                / 2,
        )
    }
...
impl SixtyCycleDay {
    ...
    pub fn get_hours(&self) -> Vec<SixtyCycleHour> {
        ...
        let t: SolarTime =
            SolarTime::from_ymd_hms(d.get_year(), d.get_month(), d.get_day(), 23, 0, 0);
...
#[cfg(test)]
mod tests {
    use crate::sixtycycle::{EarthBranch, HeavenStem, SixtyCycle};
