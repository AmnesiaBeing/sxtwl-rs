...
std::string SSQ::jieya(std::string s)
{ // 气朔解压缩
...
long double SSQ::qi_high(long double W)
{
	...
	if (v < 1200 || v > 86400 - 1200)
		t = XL::S_aLon_t(W) * 36525 - dt_T(t) + 8.0 / 24.0;
	return t;
}
...
{ // 较高精度朔
	...
	if (v < 1800 || v > 86400 - 1800)
		t = XL::MS_aLon_t(W) * 36525 - dt_T(t) + 8.0 / 24.0;
	return t;
}

long double SSQ::so_low(long double W)
...
long double SSQ::qi_low(long double W)
{ // 最大误差小于30分钟，平均5分
...
void SSQ::calcY(int jd)
{
	...
	ZQ_pe1 = calc(W - 15.2, QType);
	ZQ_pe2 = calc(W - 30.4, QType); // 补算二气,确保一年中所有月份的“气”全部被计算在内
									// 今年"首朔"的日月黄经差w
